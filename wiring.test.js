...
const mock = { profiles: [{ id: 'demo-user-1', username: 'sandy', display_name: 'Sandy K', role: 'All-rounder', batting_style: 'Right-hand bat', bowling_style: 'Right-arm medium', created_at: 'x' }], innings: [], nextId: 1000 };
...
seed.bowling.forEach((r, i) => mock.innings.push({ id: 100 + i, user_id: 'demo-user-1', kind: 'bowling', played_on: r.date, opponent: r.opponent, legal_balls: parseInt(r.overs) * 6, runs_given: r.runs_given, wickets: r.wickets, wides: r.wides, no_balls: r.no_balls, catches: r.catches, run_outs: r.run_outs, dismissal: '', created_at: 'x' }));
...
(async () => {
  ...
  ok(doc.querySelectorAll('#my-inns .inn').length === 120, 'manager lists 120 innings', doc.querySelectorAll('#my-inns .inn').length);
  ...
  await wait(300);
  ok(doc.querySelectorAll('#my-inns .inn').length === 121, 'add innings bumps list to 121');
  ...
  ok(heroStats[0] === '121' && heroStats[1] === '1178', 'public hero updates: 121 inns, 1178 runs', heroStats.join('/'));
  ok(doc.querySelector('#tab-body table.stats tr:last-child td:nth-child(4)').textContent === '75', 'HS still 75');
  ok(doc.querySelector('#tab-body table.stats tr:last-child td:nth-child(5)').textContent === '24.04', 'avg recomputed to 24.04 (55 not out)');
  ...
  await wait(300);
  ok(doc.querySelectorAll('#my-inns .inn').length === 120, 'delete returns to 120');
